#!/bin/bash
set -e

INPUT_PATH=${1:-/input/data}

echo "Creating index from: $INPUT_PATH"

# Clean up previous index data
echo "Cleaning up previous index..."
hdfs dfs -rm -r -f /indexer || true
hdfs dfs -rm -r -f /tmp/indexer || true

# Create output directories
hdfs dfs -mkdir -p /indexer

# Run MapReduce job
echo "Running MapReduce indexer..."
hadoop jar /usr/local/hadoop/share/hadoop/tools/lib/hadoop-streaming-*.jar \
    -D mapreduce.job.reduces=1 \
    -files /app/mapreduce/mapper1.py,/app/mapreduce/reducer1.py \
    -mapper mapper1.py \
    -reducer reducer1.py \
    -input $INPUT_PATH \
    -output /tmp/indexer/output

echo "MapReduce job completed!"

# Split output into separate folders by type
echo "Organizing index data..."

# Extract VOCAB lines
hdfs dfs -cat /tmp/indexer/output/part-* | grep "^VOCAB" | cut -f2- > /tmp/vocab_local.txt
hdfs dfs -mkdir -p /indexer/vocabulary
hdfs dfs -put /tmp/vocab_local.txt /indexer/vocabulary/data.txt
rm /tmp/vocab_local.txt

# Extract INDEX lines
hdfs dfs -cat /tmp/indexer/output/part-* | grep "^INDEX" | cut -f2- > /tmp/index_local.txt
hdfs dfs -mkdir -p /indexer/index
hdfs dfs -put /tmp/index_local.txt /indexer/index/data.txt
rm /tmp/index_local.txt

# Extract STATS lines
hdfs dfs -cat /tmp/indexer/output/part-* | grep "^STATS" | cut -f2- > /tmp/stats_local.txt
hdfs dfs -mkdir -p /indexer/stats
hdfs dfs -put /tmp/stats_local.txt /indexer/stats/data.txt
rm /tmp/stats_local.txt

# Cleanup temp
hdfs dfs -rm -r -f /tmp/indexer

echo "Index created successfully!"
echo ""
echo "Sample vocabulary (first 5 terms):"
hdfs dfs -cat /indexer/vocabulary/data.txt | head -5

echo ""
echo "Sample index (first 5 entries):"
hdfs dfs -cat /indexer/index/data.txt | head -5

echo ""
echo "Sample stats (first 5 docs):"
hdfs dfs -cat /indexer/stats/data.txt | head -5
